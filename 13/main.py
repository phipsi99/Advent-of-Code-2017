...
def simulate_firewallold(scanner_indexes, ranges, wait):
...
def simulate_firewall(scanner_indexes, ranges, wait):
    waits = np.arange(wait, wait + len(scanner_indexes))
    checks = (scanner_indexes + waits) % ((ranges-1) *2) == 0
    return np.where(checks)[0]
